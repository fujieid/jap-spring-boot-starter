...
public class Oauth2AutoConfiguration {
    ...
    public Oauth2Strategy oauth2Strategy(ApplicationContext applicationContext,
        ...
        try{
            ...
            Oauth2Strategy oauth2Strategy = new Oauth2Strategy(oauth2, japProperties.getBasic(), japCache);
            japTemplate.setOauth2Strategy(oauth2Strategy);
            return oauth2Strategy;
        } catch (Exception e){
            throw new JapException("尚未指定oauth2Strategy的JapUserService。若需采用该策略进行认证，请指定JapUserService实现类");
        }
